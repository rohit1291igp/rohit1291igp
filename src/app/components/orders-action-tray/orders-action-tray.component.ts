...
export class OrdersActionTrayComponent implements OnInit, OnChanges, DoCheck {
    ...
    getFeeds() {
        this.isSelectAnotherVendor = false;
        let _this = this;
        /*
        let paramsObj={
            pincode:"",
        ...
        */
        ...
        _this.BackendService.makeAjax(reqObj, function (err, response, headers) {
            ...
            // _this.dropdownData.unshift({ "Vendor_Id": "", "Vendor_Name": "Select Vendor" });
            _this.UtilityService.sharedData.dropdownData = response.result;
        });
    }
    // Check enables default vendor or all vender list
    checkValue() {
        this.isSelectAnotherVendor = !this.isSelectAnotherVendor;
    }
    //Select Vendor
    selectOption(event) {
        this.adminActions.adminActionsModel.assignChangeVendor = event;
    }
}
