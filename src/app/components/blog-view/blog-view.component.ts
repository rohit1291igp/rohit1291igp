...
export class BlogViewComponent implements OnInit, AfterViewInit {
  ...
  updateSelectedCategories(type, catData, subCatData, event) {
    ...
    if (type === 'cat') {
      ...
      if (this.blogList.data.bloglist[0].subcat[subCatData]) {
        ...
        if (
          this.selectedCategories[catData] &&
          this.selectedCategories[catData].length
        ) {
        ...
        this.selectedCategories[catData].splice(
          this.selectedCategories[catData].indexOf(subCatData),
          1
        );
      }
    }
  ...
  checkUniqueUrlValue(data) {
    ...
    if (value) {
      ...
      return false;
    } else if (!value && this.priviousvURL !== data.url) {
      this.specCharURL = false;
      const _this = this;
      const reqObj = {
        url:
          'blogs/validateblogurl?url=' +
          data.url +
          '&fkAssociateId=' +
          data.fkasid,
        method: 'get'
  ...
  saveBlog(detail) {
    ...
    data['shortdescription'] = this.replaceNonAscii(
      detail.bloglist[0].shortdescription
    );
  ...
  validateModel(data) {
    if (
      ...
      !Object.keys(this.blogList.data.bloglist[0].subcat).length
    ) {
      //
  ...
  isFeaturedImage(imageName) {
    return this.blogList.data.bloglist[0].featuredImage === imageName
      ? true
      : false;
  }
